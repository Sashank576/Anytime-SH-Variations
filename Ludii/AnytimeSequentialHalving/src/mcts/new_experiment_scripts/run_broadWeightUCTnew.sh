...
for i in "${!games[@]}"; 
    ...
    for budget in "${iteration_budget[@]}"; 
        ...
        for value in "${entropy_weights[@]}"; 
            ...
            nohup java -jar $jar_file --game "$game" --game-options $option --agents $agents --out-dir "$output_folder" --anytime-mode true --anytime-budget $budget --entropy-weight $value --thinking-time -1 --iteration-limit $budget --num-games 100 --output-alpha-rank-data --output-raw-results > "${output_folder}.out" 2> "${output_folder}.err" &
        done
    done
done
